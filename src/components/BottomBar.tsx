        ...
        <div className="h-14 flex items-center gap-4 px-4 overflow-x-auto">
          ...
          <button
            onClick={async () => {
              try {
                const port = 9502;
                const r = await fetch(`http://localhost:${port}/api/discord/connect`);
                const data = await r.json();
                if (data.success) {
                  window.open(`http://localhost:${port}/api/discord/mixer`, '_blank', 'width=320,height=600');
                } else {
                  alert('Discord connection failed: ' + (data.message || 'Is Discord running?'));
                }
              } catch { alert('CarbonBoard API not reachable'); }
            }}
            className="p-1.5 rounded transition-colors flex-shrink-0"
            style={{ background: '#5865F2' }}
            title="Discord Voice Mixer"
          >
